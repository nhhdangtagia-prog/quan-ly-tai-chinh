...
export function Wallets() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await apiFetch('/wallets/' + editId, { method: 'PUT', body: JSON.stringify(payload) });
      } else {
  ...
  return (
    <div className='p-4 flex flex-col gap-4 pb-24'>
      ...
      <div className='flex flex-col gap-3'>
        ...
        {wallets.map(w => (
          <Card key={w.id} className='flex items-center gap-4 p-4 border border-slate-700/50 bg-slate-800/50'>
